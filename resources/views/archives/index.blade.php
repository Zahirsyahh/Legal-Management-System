    <x-app-layout-dark title="Legal Archives">
        ...
        {{-- ============================================================
            FIX: Semua CSS yang berkaitan dengan badge (validity, version)
            dipindah ke x-slot name="styles" supaya masuk <head>,
            bukan di dalam $slot/<main>. Ini adalah penyebab badge
            tampil hitam-putih — style belum di-parse saat elemen render.
        ============================================================ --}}
        <x-slot name="styles">
        <style>
            /* ============================================================
            VALIDITY BADGE — 3 status: valid / expired / terminated
            Harus ada di <head>, bukan di body, agar tidak blank.
            ============================================================ */
            .validity-badge {
                display: inline-flex;
                align-items: center;
        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 animate-slide-up">
            ...
            <div id="filterPanel" class="filter-dropdown hidden mb-6 p-5 rounded-xl animate-fade-in">
                ...
                <div class="grid grid-cols-1 md:grid-cols-4 gap-6">
                    ...
                    <!-- Document Type Filter - Tampilkan SEMUA dari Archive::DOC_TYPES -->
                    <div>
                        ...
                        <table class="modern-table">
                            ...
                            <tbody>
                                ...
                                <tr class="group">
                                    ...
                                    <td>
                                        @php
                                            /* Pastikan $vs selalu 'valid', 'expired', atau 'terminated'
                                            agar cocok dengan CSS class yang sudah didefinisikan */
                                        ...
                                        <div class="flex flex-col gap-1.5">
                                            ...
                                            {{-- class="validity-badge valid/expired/terminated" --}}
        ...
        <script>
            ...
            document.addEventListener('DOMContentLoaded', function() {
                ...
                // Restore filter checkboxes dari URL
                ...
                const x = '<svg style="width:10px;height:10px;" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"/></svg>';
                let html = '';
                activeTypes.forEach(t     => { html += `<button class="filter-chip px-3 py-1.5 rounded-full bg-cyan-500/20 text-cyan-300 text-xs flex items-center gap-1.5" data-filter="type" data-value="${t}">${t} ${x}</button>`; });
                activeDepts.forEach(d     => { html += `<button class="filter-chip px-3 py-1.5 rounded-full bg-purple-500/20 text-purple-300 text-xs flex items-center gap-1.5" data-filter="dept" data-value="${d}">${d} ${x}</button>`; });
                activeVersions.forEach(v  => { html += `<button class="filter-chip px-3 py-1.5 rounded-full bg-emerald-500/20 text-emerald-300 text-xs flex items-center gap-1.5" data-filter="version" data-value="${v}">${v} ${x}</button>`; });
                activeCompanies.forEach(c => { html += `<button class="filter-chip px-3 py-1.5 rounded-full bg-amber-500/20 text-amber-300 text-xs flex items-center gap-1.5" data-filter="company" data-value="${c}">${c} ${x}</button>`; });
                ...
                document.querySelectorAll('.filter-chip').forEach(chip => {
                    chip.addEventListener('click', function() {
                        ...
                        vals.length ? url.searchParams.set(paramMap[this.dataset.filter], vals.join(',')) : url.searchParams.delete(paramMap[this.dataset.filter]);
                        url.searchParams.set('page', '1');
                        window.location.href = url.toString();
                    });
                });
