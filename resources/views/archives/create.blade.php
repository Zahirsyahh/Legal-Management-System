...
<style>
    ...
    /* ============================================================
       ...
       .hide-cr → 1 kolom (cross-ref hidden, left col full-width)
    ============================================================ */
    .two-column-layout {
        display: grid;
    ...
    /* Cross-ref sidebar — no margin-top needed: page header is outside the grid */
    .cross-ref-sidebar {
        position: relative;
        align-self: start;
        margin-top: 0.01rem; /* sejajar dengan card Classification */
    }
    ...
    .doc-status-item input:checked + .doc-status-label .check-box svg {
        opacity: 1;
    }
    ...
    .btn-submit {
        ...
        padding: 0.7rem 1.75rem; font-weight: 600; font-size: 0.875rem;
    ...
    .btn-cancel {
        background: rgba(255,255,255,0.04);
        border: 1px solid rgba(255,255,255,0.1);
        border-radius: 0.75rem; padding: 0.7rem 1.5rem;
    ...
    .alert-error {
        ...
        color: #f87171; border-radius: 0.875rem; padding: 0.875rem 1.25rem;
...
{{-- ================================================================
     PAGE HEADER — di luar grid agar sidebar bisa sejajar dgn Classification
================================================================ --}}
<div class="page-outer">
    ...
    {{-- Title row + Add Cross Reference toggle --}}
...
{{-- ================================================================
     TWO-COLUMN GRID
     Sidebar sejajar dengan card Classification (card pertama di left col)
     karena page header ada di luar grid
================================================================ --}}
<div class="two-column-layout hide-cr" id="mainLayout">
    ...
    <div class="main-form">
        ...
        <form action="{{ route('archives.store') }}" method="POST" id="archiveForm">
            ...
            {{-- SECTION 1: DOCUMENT CLASSIFICATION (includes Record ID) --}}
            <div class="form-section">
                ...
                <div class="form-grid">
                    ...
                    {{-- Record ID — pindahan dari card Record Identification --}}
                    <div class="form-group full-width">
                        ...
                        <p class="help-text">Select Company, Year, Department & Type first, then click Generate</p>
                    </div>
                    ...
                    <div class="form-group">
                        ...
                        <select id="yearSelect" class="form-select">
                            ...
                            @for($year = date('Y'); $year >= date('Y') - 20; $year--)
                                ...
                                <option value="{{ $yy }}" {{ date('Y') == $year ? 'selected' : '' }}>
                                    {{ $year }} ({{ $yy }})
                                </option>
                            @endfor
                        </select>
                        <p class="help-text">2-digit year used in Record ID</p>
                    </div>
            ...
            {{-- SECTION 2: DOCUMENT DETAILS (doc_name + doc_number pindah ke sini) --}}
            <div class="form-section">
                <div class="section-header">
                    <div class="section-title">
                        <svg fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <div class="form-grid">
                    ...
                    {{-- Document Name — pindahan dari Record Identification --}}
                    <div class="form-group full-width">
                        <label class="form-label required">Document Name</label>
                        <input type="text"
                               name="doc_name" class="form-input"
                               value="{{ old('doc_name') }}"
                               placeholder="Full document title / name"
                               required>
                    </div>

                    {{-- Document Number — pindahan dari Record Identification --}}
                    <div class="form-group">
                        <label class="form-label">Document Number</label>
                        <input type="text"
                               name="doc_number" class="form-input"
                    ...
                    <div class="form-group">
                        ...
                        <input type="text"
                               name="counterparty" class="form-input"
                    ...
                    <div class="form-group full-width">
                        ...
                        <div class="doc-status-group">
                            ...
                            <div class="doc-status-item">
                                ...
                                <label for="cs_{{ $statusValue }}" class="doc-status-label">
                                    <span class="check-box">
                                        <svg fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="3" d="M5 13l4 4L19 7"/>
                                        </svg>
                                    </span>
                                    {{ \App\Models\Archive::DOC_STATUS_LABEL[$statusValue] }}
                                </label>
                            </div>
                            @endforeach
                        </div>
                    ...
                    <div class="form-group full-width">
                        ...
                        <div class="version-radio-group">
                            ...
                            <div class="version-radio-item">
                                <input type="radio"
                                       name="version_status"
                                       id="cvs_{{ $versionValue }}"
                                       value="{{ $versionValue }}"
                                       {{ old('version_status', 'latest') == $versionValue ? 'checked' : '' }}
                                       required>
                                <label for="cvs_{{ $versionValue }}" class="version-radio-label">
                                    {{ \App\Models\Archive::VERSION_STATUS_LABEL[$versionValue] ?? ucfirst($versionValue) }}
                                </label>
                            </div>
                            @endforeach
                        </div>
            ...
            <div class="form-section">
                ...
                <div class="form-grid">
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" name="doc_location" class="form-input"
                               value="{{ old('doc_location') }}" placeholder="e.g., Cabinet A1, Shelf 3">
                    </div>
                    <div class="form-group">
                        <label class="form-label">Digital Path (Synology)</label>
                        <input type="text" name="synology_path" class="form-input"
                               value="{{ old('synology_path') }}" placeholder="/volume1/legal/2024/contracts/">
                    </div>
                </div>
            </div>
    ...
    {{-- RIGHT COLUMN — CROSS REFERENCES
         Sejajar dgn Classification karena page header di luar grid --}}
                            ...
                            <div class="cross-ref-fields">
                                ...
                                <div class="cross-ref-field">
                                    ...
                                    <input type="text" name="ref_location[]" class="cross-ref-input"
                                           value="{{ old('ref_location')[$i] ?? '' }}" placeholder="e.g., Cabinet B2">
                                </div>
                                <div class="cross-ref-field">
                                    <label class="cross-ref-label">Description / Notes</label>
                                    <textarea name="ref_relation[]" class="cross-ref-textarea"
                                              placeholder="Additional notes...">{{ old('ref_relation')[$i] ?? '' }}</textarea>
                                </div>
                            </div>
                        </div>
                        @endforeach
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    const sidebar   = document.getElementById('crossRefSidebar');
    const toggleBtn = document.getElementById('toggleCrossRefBtn');
    const closeBtn  = document.getElementById('closeCrossRefBtn');

    // Jika ada validation error dengan data cross-ref, langsung tampilkan
    @if(old('ref_doc_name'))
    showCrossRef();
    @endif
    ...
    document.getElementById('archiveForm')?.addEventListener('submit', function (e) {
        ...
        if (!document.querySelector('input[name="doc_status[]"]:checked')) {
            e.preventDefault();
            alert('Please select at least one Document Status.');
            return;
        }
    ...
    function buildCard(num) {
        ...
        div.innerHTML = `
            ...
            <div class="cross-ref-fields">
                <div class="cross-ref-field">
                    <label class="cross-ref-label required">Document Name</label>
                    <input type="text" name="ref_doc_name[]" class="cross-ref-input" placeholder="e.g., Amendment Agreement No. 3">
                </div>
                <div class="cross-ref-field">
                    <label class="cross-ref-label">Record ID</label>
                    <input type="text" name="ref_record_id[]" class="cross-ref-input" placeholder="e.g., 24LG-CT-001">
                </div>
                <div class="cross-ref-field">
                    <label class="cross-ref-label">Location</label>
                    <input type="text" name="ref_location[]" class="cross-ref-input" placeholder="e.g., Cabinet B2">
                </div>
                <div class="cross-ref-field">
                    <label class="cross-ref-label">Description / Notes</label>
                    <textarea name="ref_relation[]" class="cross-ref-textarea" placeholder="Additional notes about this reference..."></textarea>
                </div>
            </div>`;
        return div;
    }
