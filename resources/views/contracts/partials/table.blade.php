{{-- resources/views/contracts/partials/table.blade.php --}}

@if($contracts->count() > 0)
    <div class="overflow-x-auto">
        <table class="min-w-full divide-y divide-gray-800/50" id="contractsTableInner">
            <thead>
                    ...
                    <td class="px-4 py-3">
                        <div class="flex items-center justify-end gap-1" style="min-width: 140px;">
                            ...
                            @if($contract->status === 'draft' && auth()->id() === $contract->user_id)
                                <button type="button" 
                                        data-action="submit"
                                        data-url="{{ route('contracts.submit', $contract) }}"
                                        data-confirm="Submit this contract for review?"
                                        class="ajax-action-btn inline-flex items-center justify-center w-7 h-7 rounded-lg text-yellow-400 hover:text-yellow-300 
                            ...
                            @role('admin')
                                @if($contract->status === 'submitted')
                                    <button type="button" 
                                            ...
                                            data-url="{{ route('contracts.return-to-draft', $contract->id) }}"
                                            data-confirm="Return this contract to draft? This will allow the creator to edit it again."
                                            class="ajax-action-btn inline-flex items-center justify-center w-7 h-7 rounded-lg text-orange-400 hover:text-orange-300 
                            ...
                            @can('delete', $contract)
                                <button type="button" 
                                        data-action="delete"
                                        data-url="{{ route('contracts.destroy', $contract) }}"
                                        data-title="{{ addslashes($contract->title) }}"
                                        data-is-admin="{{ auth()->user()->hasRole('admin') ? 'true' : 'false' }}"
                                        class="ajax-action-btn inline-flex items-center justify-center w-7 h-7 rounded-lg text-red-400 hover:text-red-300 
                                            hover:bg-red-500/10 transition-all duration-200" 
                                        title="{{ auth()->user()->hasRole('admin') ? 'Admin Delete' : 'Delete Draft' }}">
                                    <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    {{-- 
        ...
        Both are inside #contractsTable so they get replaced on every AJAX call.
        data-summary="true" is the hook JS uses to update #resultsSummary in the parent.
    --}}
    <div class="px-6 py-4 border-t border-gray-800/30">
        <div class="flex flex-col md:flex-row items-center justify-between gap-4">

            {{-- Results summary - JS reads this to update the one outside the table --}}
            <div class="text-sm text-gray-400" data-summary="true">
                Showing
                <span class="font-medium text-gray-300">{{ $contracts->firstItem() }}</span>
                –
                <span class="font-medium text-gray-300">{{ $contracts->lastItem() }}</span>
                of
                <span class="font-medium text-gray-300">{{ $contracts->total() }}</span>
                documents
                (Page {{ $contracts->currentPage() }} of {{ $contracts->lastPage() }})
            </div>

            {{-- 
                ✅ KEY FIX: withQueryString() keeps filters on pagination links.
                The links() call renders standard <a href="?page=N&..."> anchors.
                JS intercepts those clicks via event delegation in index.blade.php.
            --}}
