...
<x-app-layout-dark title="Finance Department Admin Dashboard">
    @push('header')
        ...
        <style>
            ...
            .glossy-btn:hover::after { left: 100%; }
            .depth-blur {
            ...
            /* ============================================================ */
            /* ACTION BUTTONS — DENGAN WARNA KOTAK YANG JELAS (SAMA DENGAN TAX & ACCOUNTING) */
            /* ============================================================ */
            .btn-action {
                display: inline-flex;
                align-items: center;
    ...
    <div class="relative pb-8 px-4 sm:px-6 lg:px-8">
        ...
        <div class="fixed inset-0 overflow-hidden pointer-events-none">
            <div class="absolute -top-40 -right-40 w-80 h-80 bg-emerald-600/20 rounded-full blur-3xl animate-pulse"></div>
            <div class="absolute -bottom-40 -left-40 w-80 h-80 bg-teal-600/20 rounded-full blur-3xl animate-pulse" style="animation-delay: 2s;"></div>
            <div class="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-96 h-96 bg-green-600/10 rounded-full blur-3xl"></div>
        </div>
            ...
            <div class="flex justify-between items-center">
                <div>
                    <h1 class="text-3xl md:text-4xl font-bold">
                        <span class="gradient-text-animated">Finance Dashboard</span>
                    </h1>
                    <p class="text-gray-300 mt-2 backdrop-blur-sm inline-block px-3 py-1 rounded-full bg-white/5">
                        Welcome back, <span class="text-emerald-300 font-medium">{{ Auth::user()->name }}</span>
                    </p>
                </div>
        ...
        <div class="relative grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
            ...
            <div class="glass-stat rounded-2xl p-6">
                <div class="flex items-center justify-between">
                    <div>
                        ...
                        <p class="text-gray-400 text-xs mt-2">Waiting for review</p>
                    </div>
                ...
                <a href="{{ route('finance-admin.pending') }}" class="text-xs text-yellow-300/80 hover:text-yellow-300 mt-4 inline-flex items-center gap-1 transition-all">
                    View details
            ...
            <div class="glass-stat rounded-2xl p-6">
                <div class="flex items-center justify-between">
                    ...
                    <div class="w-12 h-12 bg-emerald-500/10 rounded-xl flex items-center justify-center backdrop-blur-sm border border-emerald-500/20">
                        <svg class="w-6 h-6 text-emerald-300" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z"/>
                        </svg>
                    </div>
                </div>
                <a href="{{ route('finance-admin.active') }}" class="text-xs text-emerald-300/80 hover:text-emerald-300 mt-4 inline-flex items-center gap-1 transition-all">
                    View details
                    <svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7"/>
                    </svg>
        ...
        <div class="relative grid grid-cols-1 lg:grid-cols-2 gap-6 mb-8">
            ...
            <div class="glass-card rounded-2xl overflow-hidden">
                <div class="px-6 py-4 border-b border-white/10 flex justify-between items-center">
                    ...
                    <a href="{{ route('finance-admin.pending') }}" class="text-sm text-yellow-300/80 hover:text-yellow-300 transition-all">
                        View All →
                    </a>
                </div>
                                ...
                                <tr class="hover:bg-white/5 transition-colors duration-200">
                                    ...
                                    <td class="px-6 py-4">
                                        <div class="flex items-center justify-center gap-2">
                                            ...
                                            {{-- Accept Button - Hijau --}}
                                            <form method="POST"
                                                  action="{{ route('finance-admin.invitation.accept', $assignment) }}"
                                            ...
                                            <form id="decline-form-{{ $assignment->id }}" method="POST"
                                                  action="{{ route('finance-admin.invitation.decline', $assignment) }}"
                                                  class="hidden">
                                                @csrf
                ...
                @if($pendingAssignments->count() > 5)
                    <div class="px-6 py-3 border-t border-white/10 text-center">
                        <a href="{{ route('finance-admin.pending') }}" class="text-xs text-yellow-300/70 hover:text-yellow-300 transition-all">
            ...
            <div class="glass-card rounded-2xl overflow-hidden">
                <div class="px-6 py-4 border-b border-white/10 flex justify-between items-center">
                    <div>
                        ...
                        <p class="text-xs text-gray-400 mt-1">Currently being reviewed by finance staff</p>
                    </div>
                    <a href="{{ route('finance-admin.active') }}" class="text-sm text-emerald-300/80 hover:text-emerald-300 transition-all">
                        View All →
                    </a>
                </div>
                <div class="overflow-x-auto">
                        ...
                        <tbody class="divide-y divide-white/5">
                            @forelse($activeReviews->take(5) as $review)
                                <tr class="hover:bg-white/5 transition-colors duration-200">
                                    ...
                                    <td class="px-6 py-4">
                                        <span class="inline-flex items-center px-2.5 py-1 rounded-full text-xs font-medium backdrop-blur-sm
                                    ...
                                    <td class="px-6 py-4">
                                        <a href="{{ route('contracts.show', $review->contract) }}"
                                           class="glossy-btn px-4 py-2 text-xs bg-emerald-500/20 hover:bg-emerald-500/30 text-emerald-300
                                                  rounded-lg transition-all inline-flex items-center gap-2 border border-emerald-500/30">
                ...
                @if($activeReviews->count() > 5)
                    <div class="px-6 py-3 border-t border-white/10 text-center">
                        <a href="{{ route('finance-admin.active') }}" class="text-xs text-emerald-300/70 hover:text-emerald-300 transition-all">
        ...
        <div class="relative grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            <div class="glass-card rounded-2xl p-6">
                <h2 class="text-lg font-semibold text-white mb-4">Finance Admin Tools</h2>
                ...
                <div class="space-y-4">
                    <div class="grid grid-cols-2 gap-4">
                        <div class="bg-white/5 rounded-xl p-3 backdrop-blur-sm border border-white/5">
                            ...
                            <p class="text-2xl font-bold text-white">
                                {{ \App\Models\TblUser::whereHas('roles', fn($q) => $q->where('name', 'staff_fin'))->where('status_karyawan', 'AKTIF')->count() }}
                            </p>
                        </div>
                        <div class="bg-white/5 rounded-xl p-3 backdrop-blur-sm border border-white/5">
                    ...
                    <div>
                        ...
                        <div class="w-full bg-white/10 rounded-full h-2 overflow-hidden">
                            <div class="bg-gradient-to-r from-emerald-400 to-teal-400 h-2 rounded-full transition-all duration-700"
                        ...
                        <div class="space-y-2">
                            ...
                            @if($activeCount > 0)
                                <div class="flex items-center justify-between text-sm">
                                    ...
                                    <span class="text-emerald-300 font-medium">{{ $activeCount }}</span>
                                </div>
                            @endif
    ...
    @if(session('invitation_declined'))
        <div id="inv-declined-toast"
            ...
            <div class="flex items-start gap-4 px-5 py-4 rounded-2xl border border-red-500/40 shadow-2xl"
                 style="background: linear-gradient(135deg, rgba(185,28,28,0.3), rgba(239,68,68,0.15)); backdrop-filter: blur(20px);">
                <div class="w-10 h-10 rounded-full bg-red-500/20 border border-red-500/40 flex items-center justify-center flex-shrink-0">
                ...
                <div class="flex-1 min-w-0">
                    ...
                    <p class="text-red-200/80 text-xs mt-0.5 break-words">{!! session('invitation_declined') !!}</p>
                </div>
        ...
        <script>setTimeout(() => document.getElementById('inv-declined-toast')?.remove(), 6000);</script>
    @endif
    ...
    @if(session('invitation_accepted'))
        <div id="inv-accepted-toast"
             class="fixed top-5 left-1/2 -translate-x-1/2 z-[9999] w-full max-w-lg px-4"
             style="animation: slideDownToast 0.4s ease;">
            <div class="flex items-start gap-4 px-5 py-4 rounded-2xl border border-emerald-500/40 shadow-2xl"
                 style="background: linear-gradient(135deg, rgba(5,150,105,0.35), rgba(16,185,129,0.2)); backdrop-filter: blur(20px);">
                <div class="w-10 h-10 rounded-full bg-emerald-500/25 border border-emerald-500/40 flex items-center justify-center flex-shrink-0">
                    <svg class="w-5 h-5 text-emerald-300" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"/>
                    </svg>
                </div>
                <div class="flex-1 min-w-0">
                    <p class="text-white font-semibold text-sm">Joined Workflow!</p>
                    <p class="text-white/80 text-xs mt-0.5 break-words">{!! session('invitation_accepted') !!}</p>
                </div>
                <button onclick="document.getElementById('inv-accepted-toast').remove()"
                        class="text-white/50 hover:text-white transition-colors flex-shrink-0 mt-0.5">
                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"/>
                    </svg>
                </button>
        ...
        <script>setTimeout(() => document.getElementById('inv-accepted-toast')?.remove(), 6000);</script>
    @endif
