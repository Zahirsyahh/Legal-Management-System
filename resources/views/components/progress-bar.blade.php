...
@php
    ...
    // ── BARU: Load revision task counts per stage ──
    // Gunakan eager loading agar tidak N+1
    $stageIds = $stages->pluck('id')->toArray();

    // Revision tasks yang dikirim DARI stage (stage sebagai pemberi revisi / Bimo)
    ...
    // Revision tasks yang diterima OLEH stage (stage sebagai penerima / Bunga, Sari)
    ...
    // Helper: hitung berapa submitted (menunggu keputusan Bimo)
...
<div class="progress-bar-container" style="position: sticky; top: 0; z-index: 5; background: inherit; padding: 1rem 0; margin-bottom: 1rem;">
    <div class="glass-card rounded-xl p-6 overflow-visible" style="position: relative; z-index: 5;">
                ...
                @foreach($displayItems as $index => $item)
                    @if($item['type'] === 'sequential')
                        @php
                            ...
                            // Hitung badge untuk stage ini
                            $sentOpen      = isset($sentTaskCounts[$stage->id])
                                ? $sentTaskCounts[$stage->id]->sum('total')
                                : 0;
                            $sentSubmitted = $submittedTaskCounts[$stage->id] ?? 0;
                            $receivedOpen  = $receivedTaskCounts[$stage->id] ?? 0;

                            // Stage ini punya tugas revisi yang harus dikerjakan (penerima)
                            $hasIncomingRevision = $receivedOpen > 0;
                            // Stage ini menunggu hasil revisi dari orang lain (pengirim)
                        ...
                        <div class="flex flex-col items-center relative overflow-visible">
                            <div class="relative mb-2 overflow-visible">
                                {{-- Ping indicator untuk active stage --}}
                                @if($stage->isActive())
                                ...
                                {{-- Badge: ada revisi masuk (penerima harus kerjakan) --}}
                                ...
                                {{-- Badge: ada hasil revisi yang perlu ditinjau (submitted, menunggu Bimo) --}}
                                @if($sentSubmitted > 0)
                            ...
                            <div class="text-center max-w-[120px]">
                                ...
                                {{-- Status label dengan kondisi khusus untuk executing/archiving --}}
                                <p class="text-xs mt-1
                                    ...
                                    @else text-gray-500
                                    @endif">
                                    @if($stage->stage_type === 'executing' && $stage->status === 'completed')
                                        Executed
                                    @elseif($stage->stage_type === 'archiving' && $stage->status === 'completed')
                                        Archived
                                    @elseif($stage->status === 'revision_requested')
                                        Revision
                        ...
                        <div class="parallel-group-wrapper flex flex-col items-center relative overflow-visible">
                            <div class="relative mb-2 overflow-visible">
                                <div class="w-10 h-10 rounded-full flex items-center justify-center relative z-10 transition-all duration-200
                                    ...
                                    @elseif($group['stages']->contains(fn($s) => $s->isActive())) bg-purple-500 text-white border-2 border-purple-300 shadow-lg shadow-purple-500/30
                            ...
                            <div class="text-center max-w-[140px]">
                                ...
                                <p class="text-xs text-gray-400 mt-1">{{ $group['stages']->where('status', 'completed')->count() }}/{{ $group['stages']->count() }} Done</p>
                            </div>
                        </div>
                    @endif
                @endforeach
                            ...
                            @if($item['type'] === 'sequential')
                                ...
                                <div class="flex flex-col items-center" style="min-width: 140px;">
                                    ...
                                    <div class="text-center w-[140px]">
                                        ...
                                        {{-- Status label dengan kondisi khusus untuk executing/archiving --}}
                                        <p class="text-xs mt-1 @if($stage->status === 'completed') text-green-400 @elseif($stage->isActive()) text-blue-400 @else text-gray-500 @endif">
                                            @if($stage->stage_type === 'executing' && $stage->status === 'completed')
                                                Executed
                                            @elseif($stage->stage_type === 'archiving' && $stage->status === 'completed')
                                                Archived
        ...
        @endphp
        @if($totalIncoming > 0 || $totalSubmitted > 0 || $totalOpenSent > 0)
            ...
            <div class="parallel-dropdown hidden" data-group="{{ $groupId }}" 
                ...
                <div class="relative p-3">
                    ...
                    <div class="space-y-2 max-h-[300px] overflow-y-auto">
                        ...
                        @endphp
                        <div class="border-l-2 {{ $parStage->status === 'completed' ? 'border-green-500' : ($parStage->isActive() ? 'border-blue-500' : 'border-gray-600') }} pl-3">
                            <div class="flex items-center justify-between mb-1">
                                ...
                                <div class="flex items-center gap-1">
                                    ...
                                    <span class="text-xs px-1.5 py-0.5 rounded-full 
                                        ...
                                        {{ $parStage->status === 'completed' ? 'Done' : ($parStage->isActive() ? 'Active' : ($parStage->status === 'assigned' ? 'Assigned' : 'Pending')) }}
                                    </span>
                                </div>
                            </div>
                            <p class="text-gray-300 text-sm">
                                <span class="text-gray-500">Reviewer:</span> {{ $parStage->assignedUser->nama_user ?? 'Not Assigned' }}
                            </p>
