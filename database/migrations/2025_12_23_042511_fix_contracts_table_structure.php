...
return new class extends Migration
{
    public function up()
    {
        Schema::table('contracts', function (Blueprint $table) {
            ...
            if (!Schema::hasColumn('contracts', 'contract_value')) {
                $table->decimal('contract_value', 15, 2)->nullable()->after('drafting_deadline');
            }
            ...
            // FIELD UNTUK REVIEW SYSTEM
            if (!Schema::hasColumn('contracts', 'current_stage')) {
                $table->integer('current_stage')->default(1)->nullable();
            }
            
            if (!Schema::hasColumn('contracts', 'current_stage_id')) {
                $table->unsignedBigInteger('current_stage_id')->nullable()->after('current_stage');
            }
            ...
            // ASSIGNMENT
            if (!Schema::hasColumn('contracts', 'legal_assigned_id')) {
                $table->unsignedBigInteger('legal_assigned_id')->nullable()->after('user_id');
            }

            if (!Schema::hasColumn('contracts', 'fat_assigned_id')) {
                $table->unsignedBigInteger('fat_assigned_id')->nullable()->after('legal_assigned_id');
            }

            // REVIEW STATE FLAG
            if (!Schema::hasColumn('contracts', 'legal_reviewing')) {
                $table->boolean('legal_reviewing')->default(false);
            }

            if (!Schema::hasColumn('contracts', 'fat_reviewing')) {
                $table->boolean('fat_reviewing')->default(false);
            }
        });
    }
    ...
    {
        Schema::table('contracts', function (Blueprint $table) {
            // Hapus semua field yang kita tambahkan (jika ada)
            $columnsToDrop = [];
            
            $newColumns = [
                'contract_type', 'counterparty_name', 'counterparty_email', 
                'counterparty_phone', 'effective_date', 'expiry_date',
                'drafting_deadline', 'contract_value', 'currency', 'additional_notes',
                'current_stage', 'current_stage_id', 'review_flow_status',
                'legal_review_started_at', 'legal_review_completed_at', 'legal_notes',
                'legal_status', 'submitted_at', 'legal_reviewed_at', 'legal_feedback', 'legal_assigned_id',
                'fat_assigned_id', 'legal_reviewing', 'fat_reviewing'
            ];
